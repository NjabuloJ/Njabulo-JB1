...
ezra({
  nomCom: 'anticall',
  categorie: "NJABULO-SETTING"
}, async (chatId, zk, context) => {
  const { ms, repondre, superUser, auteurMessage, arg } = context;

  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner. or Njabulo owner* 🤦,,idiot");
  }
...
ezra({
  ...
  categorie: "NJABULOJB-SETTING"
}, async (chatId, zk, context) => {
  const { ms, repondre, superUser, auteurMessage, arg } = context;

  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner or Niabulo owner.* 🤦,,idiot");
  }
...
ezra({
  ...
  const { ms, repondre, superUser, auteurMessage,
