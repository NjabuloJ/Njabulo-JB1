...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (!superUser) {
    return repondre("Only for my owner or Njabulo JB Tech to execute this command 🚫");
  }
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or Njabulo JB to execute 🚫`;
      ...
      if (connection === "connecting") {
        ...
        if (conf.DP.toLowerCase() === 'yes') {
          await zk.sendMessage(zk.user.id, {
            text: `*NJABULO JB* connected successfully ✅
